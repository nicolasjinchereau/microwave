...
inline void Stream::Ignore(size_t bytes) {
    Seek(bytes, SeekOrigin::Current);
}

inline void Stream::SetPosition(size_t position) {
    Seek(position, SeekOrigin::Begin);
}

inline Task<void> Stream::FlushAsync() {
    co_await ThreadPool::InvokeAsync([this] { Flush(); });
}

inline Task<int> Stream::ReadAsync(std::span<std::byte> buffer)
{
    int read = 0;
...
inline Task<void> Stream::WriteAsync(std::span<std::byte> buffer)
{
    co_await ThreadPool::InvokeAsync([&] { Write(buffer); });
}

inline void Stream::CopyTo(sptr<Stream>& stream)
{
...
inline Task<void> Stream::CopyToAsync(sptr<Stream>& stream) {
    co_await ThreadPool::InvokeAsync([&] { CopyTo(stream); });
}

template<class T>
inline T Stream::ReadValue()
{
    T ret;
...
template<class T>
inline bool Stream::ReadValue(T& output)
{
...
template<class T>
inline void Stream::WriteValue(const T& value) {
