...
HWRenderTextureD3D11::HWRenderTextureD3D11(const gptr<HWTexture>& backingTex)
    : tex(gpcast<HWTextureD3D11>(backingTex))
{
	if(!tex)
	...
	auto context = gpcast<HWContextD3D11>(graphics->context);
	...
	if(FAILED(res))
		throw Exception("render texture creation failed");
...
gptr<HWTexture> HWRenderTextureD3D11::GetTexture() {
	return tex;
}
