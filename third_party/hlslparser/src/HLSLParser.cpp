...
bool HLSLParser::Parse(HLSLTree* tree, ParserError* pError)
{
    m_tree = tree;
    ...
    while (!Accept(HLSLToken_EndOfStream))
    {
        ...
        {
            if (pError && m_tokenizer.HasError())
            {
                pError->line = m_tokenizer.GetErrorLineNumber();
                pError->msg = m_tokenizer.GetErrorMessage();
            }
